...
module('Integration | Component | changeset-form', function(hooks) {
  ...
  test('it renders form', async function(assert) {
    ...
    assert.equal(find('[data-test-submit]').disabled, true, 'button disabled');

    // submit - disabled until fill in proper email
    await click('[data-test-submit]');

    assert.equal(find('[data-test-submit]').disabled, true, 'button disabled');
    assert.equal(find('[data-test-model-user-email]').textContent.trim(), 'something', 'has old email');
    ...
    assert.equal(find('[data-test-submit]').disabled, false, 'button still disabled');
    ...
    assert.equal(find('[data-test-cid]').value, '2', 'has cid input value');
    assert.equal(find('[data-test-user-email]').value, 'foo@gmail.com', 'has email input value');
